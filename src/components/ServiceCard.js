...
export const ServiceCard = ({ name, title, idx, description, icon: Icon, setActiveService, activeService, setPrev }) => {
  ...
  return (
    <>
      ...
      >
        ...
        {isActive && <div className={` absolute h-full w-full top-0 left-0 bg-black z-0 transition-all}`}></div>}
        ...
        <div className="flex flex-col gap-5  sm:gap-2 z-10 font-semibold ">
          <h3 className={`text-center lg:text-start text-2xl sm:text-lg group-hover:text-white text-black ${isActive ? "text-white" : ""}`}>
            {/* {capitalize(t(name))} */}
            {name}
          </h3>
          <button to="#" className="justify-center lg:justify-between flex items-center  text-black transition-all ">
      ...
      <Collapse isOpened={isActive}>
        <div className={`bg-black text-white p-8 font-heading sm:hidden`}>{description}</div>
      </Collapse>
    </>
  )
}
